...
pub fn tokenize_name(mut cx: FunctionContext) -> JsResult<JsValue> {
    ...
    let context: crate::types::InputContext = neon_serde::from_value(&mut cx, context).or_else(|e| cx.throw_error("some error message"))?;
    let context = crate::Context::from(context);
    let tokenized = context.tokens.process(&name, &context.country);

    Ok(neon_serde::to_value(&mut cx, &tokenized).or_else(|e| cx.throw_error("some error message"))?)
}
